...
class ClearComponents extends Command
{
    ...
    public function handle(Filesystem $filesystem): int
    {
        ...
        $this->cleanAutoGeneratedBladeViews($filesystem);
    ...
    private function cleanAutoGeneratedBladeViews(Filesystem $filesystem): void
    {
        foreach (config('view.paths') as $path) {
            if ($filesystem->exists($directory = $path.'/components/splade') === false) {
                continue;
            }
            ...
            foreach ($filesystem->allFiles($directory) as $script) {
                ...
                /** @var SplFileInfo $script */
                if (! str_starts_with($filename, 'auto-generated-') || ! str_ends_with($filename, '.blade.php')) {
                    continue;
                }
