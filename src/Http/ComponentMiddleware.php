...
{
    public static array $ignoredMiddleware = [];
     ...
     */
    protected function gatherMiddleware(string $url, string $method): array
    {
        $originalRequest = $this->makeRequestFromUrlAndMethod($url, $method);
     ...
     * Middleware that is already applied to the 'splade-core.invoke-component' route.
     */
    public function resolveApplicableMiddleware(string $url, string $method): array
    {
        ...
        $handlerRoute = $router->getRoutes()->getByName('splade-core.invoke-component');
        ...
        return Collection::make($originalMiddleware)->reject(function ($middleware) use ($handlerMiddleware) {
            ...
            return in_array($middlewareClass, $handlerMiddleware) || in_array($middlewareClass, self::$ignoredMiddleware);
        })->values()->all();
    }
